 ...
 */
export async function resolveChunkedImportZip(inputPath: string): Promise<{
  zipBytes: Uint8Array;
  rootPath: string;
} | null> {
  const resolved = path.resolve(inputPath);
  ...
  if (resolvedStat.isFile() && path.extname(resolved).toLowerCase() === ".zip") {
    ...
    if (resolvedStat.size > CHUNKED_IMPORT_THRESHOLD_BYTES) return { zipBytes, rootPath };
    ...
    if (estimateInlineImportBytes(archive.files) <= CHUNKED_IMPORT_THRESHOLD_BYTES) return null;
  ...
  if (contentBytes <= CHUNKED_IMPORT_THRESHOLD_BYTES) {
    ...
    if (estimateInlineImportBytes(inlineEntries) <= CHUNKED_IMPORT_THRESHOLD_BYTES) return null;
  }
      ...
      .action(async (fromPathOrUrl: string, opts: CompanyImportOptions) => {
        try {
          ...
          if (isHttpUrl(from) || isGithubSource) {
            ...
            chunkedZip = await resolveChunkedImportZip(from);
            if (chunkedZip) {
