...
export function companyRoutes(db: Db, storage?: StorageService, options?: CompanyRoutesOptions) {
  ...
  router.put(
    ...
    async (req, res) => {
      ...
      if (!recorded) {
        // The run went terminal between the load above and this record — most
        // likely the abandoned-spool sweep expired it and deleted the rest of
        // the spool. Drop the stray part just written and tell the client the
        // transfer is dead instead of silently recording nothing.
