...
When a delivery courier, shepherd, poller, or source-review issue registers the same structured PR as its source issue, include the trusted link in that create call: `{"type":"pull_request","provider":"github","externalId":"321","title":"paperclipai/paperclip#321","url":"https://github.com/paperclipai/paperclip/pull/321","status":"ready_for_review","deliveryResidueLink":{"sourceIssueId":"<source-issue-uuid>","originKind":"delivery_courier"}}`. The source issue must already contain the exact same structured PR. Paperclip atomically validates and stamps the origin, creates the product, and records the audit; mismatches return `422` and create nothing. Raw origin fields are never client-settable.
